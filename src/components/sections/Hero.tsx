...
const Hero: React.FC<HeroProps> = ({ onAnimationComplete }) => {
  ...
  // Handle resume download with feedback
  ...
  useEffect(() => {
    if (!heroRef.current || prefersReducedMotion) {
      ...
      if (heroRef.current) {
        gsap.set(heroRef.current.querySelectorAll('.animate-element'), { opacity: 1, y: 0, scale: 1 })
      }
      onAnimationComplete?.()
      return
    }
...
<h2 className="hero-subtitle animate-element text-2xl md:text-4xl font-bold text-gray-700 dark:text-gray-300 mb-6">
  MERN Stack Developer • <span className="text-blue-600">Frontend Specialist</span>
</h2>

<p className="hero-description animate-element text-lg text-gray-600 dark:text-gray-400 mb-8 max-w-2xl lg:mx-0 mx-auto">
  I build scalable web applications with the **MERN** stack. My core expertise lies in crafting 
  **high-performance React interfaces** that are clean, responsive, and user-centric. 
  I bridge the gap between complex backend architecture and seamless user experiences.
</p>
...
<div className="lg:w-1/2 flex justify-center">
  <div className="relative">
    {/* Profile Image */}
    <div 
      className="hero-image animate-element w-80 h-80 rounded-full overflow-hidden shadow-2xl bg-gradient-to-br from-blue-400 to-purple-500 p-2"
      role="img"
...
<div 
  className="floating-icon floating-icon-1 absolute -top-6 -right-6 w-16 h-16 bg-white dark:bg-gray-800 rounded-2xl shadow-xl flex items-center justify-center border border-cyan-100 dark:border-cyan-900/30"
  title="React.js Specialist"
>
  <svg className="w-10 h-10 text-[#61DAFB] animate-[spin_15s_linear_infinite]" viewBox="-11.5 -10.23174 23 20.46348" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<div 
  className="floating-icon floating-icon-2 absolute top-1/2 -left-10 w-14 h-14 bg-white dark:bg-gray-800 rounded-2xl shadow-xl flex items-center justify-center border border-sky-100 dark:border-sky-900/30"
  title="Tailwind CSS"
>
  <svg className="w-8 h-8 text-[#38BDF8]" fill="currentColor" viewBox="0 0 24 24">
...
<div 
  className="floating-icon floating-icon-3 absolute -bottom-6 left-1/4 w-14 h-14 bg-white dark:bg-gray-800 rounded-2xl shadow-xl flex items-center justify-center border border-yellow-100 dark:border-yellow-900/30"
